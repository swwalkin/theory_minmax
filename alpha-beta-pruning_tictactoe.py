...
def Search(board, depth, turn, alpha, beta):
    ...
    if turn:
        ...
        for i in range(Board_Size**2):
            if board[i] == " ":
                board[i] = "O"
                score = Search(board, depth + 1, False, alpha, beta)
                board[i] = " "
        ...
        for i in range(Board_Size**2):
            if board[i] == " ":
                board[i] = "X"
                score = Search(board, depth + 1, True, alpha, beta)
                board[i] = " "
...
def minmax():
    ...
    for i in range(Board_Size**2):
        if Board[i] == " ":
            Board[i] = "O"
            score = Search(Board, 0, False, alpha, float("inf"))
            Board[i] = " "
